...
O **host** utiliza o **guest**, então ele deve fornecer os inputs requeridos. Além disso, o host também pode realizar alguma ação quando for notificado de que o usuário deseja realizar uma exclusão ou uma edição de uma disciplina.
...
Por fim, a sintaxe que tem um atributo HTML com nome entre parênteses você também já conhece. É a mesma que já utilizamos para tratar eventos \(ex: excluir uma disciplina quando o usuário clicar em um botão associado a ela\). Então o trecho \(onEditar\)="editar\($event\)" indica o uso de um tratador de eventos: o método editar\(\) deve ser chamado passando $event como parâmetro quando ocorrer o evento onEditar do ListaDeDisciplinas. É importante observar que $event é uma forma de 
